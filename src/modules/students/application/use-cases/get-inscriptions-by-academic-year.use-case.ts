...
@Injectable()
export class GetInscriptionsByAcademicYearUseCase {
  constructor(
  ...
  async execute(academicYearId: string): Promise<InscriptionResponse[]> {
    const inscriptions = await this.inscriptionRepository.findByAcademicYear(academicYearId);
    ...
    return inscriptions.map(inscription => ({
      ...
      className: inscription.classYear?.class?.name,
      academicYearName: inscription.academicYear?.name,
